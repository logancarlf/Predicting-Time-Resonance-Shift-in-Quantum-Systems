...
class System:
    ...
    def excitation_probability(self, omega, Omega):
        '''
        ...
        fd = np.sqrt((omega - Omega)**2 + self.__Lambda**-2)
        # excitation probability is square of resonance factor
        return (fn/fd) ** 2

    def theoretical(self, theta_j, Omega, discrete=False, plot=False):
        '''
        ...
        prob = list()
        for i in x:
            p = self.excitation_probability(i, Omega)
            prob.append(p)
